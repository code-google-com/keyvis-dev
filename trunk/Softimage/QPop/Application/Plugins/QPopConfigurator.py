 ...
 # Declare list of exported functions:
	...
	def deleteSep (self,sep):
		...
		try:
			...
			Print(sep.name + "could not be found in QPop globals - nothing to delete!", c.siWarning)
...
#========================= Advanced Configuration Tab =============================================================================	
	...
	oLayout.AddGroup("Assigned QPop Menu Set(s) to selected View")
	...
	oLayout.AddGroup ("Assigned QPop Menu Contexts to selected Menu Set")
	oLayout.AddRow()
...
def QPopConfigurator_CreateNewScriptItem_OnClicked():
	...
	if Language > -1: #User did not press Cancel?
		...
		if Language == 0: newMenuItem.code = ("def Script_Execute (self, QPopMenuItems, QPopMenus, QPopMenuSets): #Dont rename this function \n\t#Put your script code here\n\tpass"); newMenuItem.language = "Python"
		if Language == 1: newMenuItem.code = ("function Script_Execute (self, QPopMenuItems, QPopMenus, QPopMenuSets) {\n\t//Put your script code here\n\tdoNothing = true\n}"); newMenuItem.language = "JScript"
		if Language == 2: newMenuItem.code = ("Function Script_Execute (self, QPopMenuItems, QPopMenus, QPopMenuSets) \n\t' Put your script code here\n\tdoNothing = true\nend Function"); newMenuItem.language = "VBScript"
...
def QPopConfigurator_CreateNewSwitchItem_OnClicked():
	...
	if LanguageNumber > -1:
		...
		#Set default code
		if Language == "Python": 
			newMenuItem.code = ("def Switch_Init (self, QPopMenuItems, QPopMenus, QPopMenuSets): #Don't rename this function\n\t#Add your code here, return value must be boolean and represent the current state of the switch (on or off)\n\treturn False\n\n")
			newMenuItem.code += ("def Switch_Execute (self, QPopMenuItems, QPopMenus, QPopMenuSets): #Don't rename this function\n\t#Add your code here, it gets executed when the switch item is clicked in a QPop menu \n\tpass\n\n")
		
		if Language == "JScript": 
			newMenuItem.code = ("function Switch_Init (self, QPopMenuItems, QPopMenus, QPopMenuSets) //Don't rename this function\n{\n\t//Add your code here, return value must be boolean and represent the current state of the switch (on or off)\n\treturn false\n}\n\n")
			newMenuItem.code += ("function Switch_Execute (self, QPopMenuItems, QPopMenus, QPopMenuSets) //Don't rename this function\n{\n\t//Add your code here, it gets executed when the switch item is clicked in a QPop menu \n}")
...
def QPopConfigurator_ExecuteCode_OnClicked():
	...
	if PPG.Menus.Value != "":
		...
		if oSelectedItem != None:
			...
			if Code != "":
				ArgList = [oSelectedItem, globalQPopMenuItems, globalQPopMenus, globalQPopMenuSets]
				try:
					App.ExecuteScriptCode(Code, Language,"QPopMenu_Execute", ArgList)
				except:
					Print("An Error occured executing the script code of QPop Menu '" + oSelectedItem.name + "', please see script editor for details!", c.siError)
					raise
				return
...
def QPopConfigurator_ItemUp_OnClicked():
	...
	oMenu = getQPopMenuByName(PPG.MenuChooser.Value) #Lets get the name of the menu currently selected in the menu chooser drop down list
...
def DisplayMenuSet( MenuSetIndex ):
	...
	if globalQPopViewSignatures != None:
		...
		if oMenuSet != None:
			...
			QPopMenus = QPopGlobals("globalQPopMenus").items
			QPopMenuSets = QPopGlobals("globalQPopMenuSets").items
			#ArgList.append(QPopMenuSets)
			#ArgList.append(QPopMenuItems)
			#ArgList.append(QPopMenus)
			...
			for RuleIndex in range(0,len(oMenuSet.AContexts)):
				...
				if Language == "Python":
					...
					DisplayMenu = App.ExecuteScriptCode( oMenuSet.AContexts[RuleIndex].code, Language, "QPopContext_Execute",[selection, Types, ClassNames, ComponentClassNames, ComponentParents,ComponentParentTypes,ComponentParentClassNames]) #This function returns a variant containing the result of the executed function and...something else we don't care about 
					if Language == "VBScript":
						DisplayMenu = DisplayMenu[0]
			...
			for RuleIndex in range(0,len(oMenuSet.BContexts)):
				...
				if Language == "Python":
					...
					DisplayMenu = App.ExecuteScriptCode( oMenuSet.BContexts[RuleIndex].code, Language, "QPopContext_Execute",[selection, Types, ClassNames, ComponentClassNames, ComponentParents,ComponentParentTypes,ComponentParentClassNames]) #This function returns a variant containing the result of the executed function and...something else we don't care about 
				if DisplayMenu:
			...
			for RuleIndex in range(0,len(oMenuSet.CContexts)):
				...
				if Language == "Python":
					...
					DisplayMenu = App.ExecuteScriptCode( oMenuSet.CContexts[RuleIndex].code, Language, "QPopContext_Execute",[selection, Types, ClassNames, ComponentClassNames, ComponentParents,ComponentParentTypes,ComponentParentClassNames]) #This function returns a variant containing the result of the executed function and...something else we don't care about 
			...
			for RuleIndex in range(0,len(oMenuSet.DContexts)):
				...
				if Language == "Python":
					...
					DisplayMenu = App.ExecuteScriptCode( oMenuSet.DContexts[RuleIndex].code, Language, "QPopContext_Execute",[selection, Types, ClassNames, ComponentClassNames, ComponentParents,ComponentParentTypes,ComponentParentClassNames]) #This function returns a variant containing the result of the executed function and...something else we don't care about 
				if DisplayMenu:
							...
							for oItem in oMenu.items:
								...
								if oItem.type == "QPopMenuItem":
									...
									if oItem.switch == True:
										...
										self = oItem
										if Language == "Python":
											Code = (oItem.code + ("\nresult = Switch_Init(self, QPopMenuItems, QPopMenus, QPopMenuSets)"))
											exec (Code)
										else:
											results = App.ExecuteScriptCode(oItem.code, Language, "Switch_Init",[self, QPopMenuItems, QPopMenus, QPopMenuSets])
											result = results[0]
										if result == True:
...
def InitQPop_OnEvent (in_ctxt):
	...
	if (FirstStartup == "True") or (FirstStartup == "1") or (FirstStartup == True) or (FirstStartup == 1):
		...
		QPopConfigFile = GetDefaultConfigFilePath("QPopConfiguration_Default.xml") #Get the file path as string of the QPop default configuration file.
